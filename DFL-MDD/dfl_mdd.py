...
def backtest_dfl_mdd(pred_model, opt_layer, rebal_samples, N, d, C,
    ...
    for i, (z_np, r_np) in enumerate(rebal_samples):
        ...
        y_real = compute_cumulative_path(r_real)
        w_real = compute_realized_path(x_star, y_real)[0].numpy()   # (N,)
...
# class PredictionModel(nn.Module):
#     def __init__(self, input_dim, hidden_dim, N, m):
#         super().__init__()
#         self.N = N
#         self.m = m
#         self.net = nn.Sequential(
...
#     def forward(self, z):
#         batch = z.shape[0]
#         return self.net(z).view(batch, self.N, self.m)

# def compute_cumulative_path(r):
#     return torch.cumsum(r, dim=1)

# def build_optimization_layer(N, m):
#     x     = cp.Variable(m,     name="x")
#     u     = cp.Variable(N + 1, name="u")
#     Y_hat = cp.Parameter((N, m), name="Y_hat")
#     n1C   = cp.Parameter(nonneg=True, name="n1C")
...
# def solve_portfolio(y_hat, opt_layer, n1, C, x_min, x_max):
#     batch, N, m = y_hat.shape
#     n1C_val   = torch.tensor(n1 * C, dtype=torch.float64)
#     x_min_val = torch.tensor(x_min,  dtype=torch.float64)
#     x_max_val = torch.tensor(x_max,  dtype=torch.float64)
...
# def compute_realized_path(x_star, y_real):
#     return torch.einsum("bj, btj -> bt", x_star, y_real)

# def compute_return(w_real, d, C):
#     return w_real[:, -1] / (d * C)

# def compute_max_drawdown(w_real):
#     running_max, _ = torch.cummax(w_real, dim=1)
#     return torch.max(running_max - w_real, dim=1).values

# def dfl_loss(R_real, M_real, lam):
...
# # =============================================================================
# # plot_pnl — label 파라미터로 DFL / PTO-MDD / PTO-MVO 구분
# # =============================================================================
# def plot_pnl(bt_results: list, horizon: int, label: str = "Portfolio", figsize=(12, 6)):
#     pv            = [1.0]
#     rebal_indices = [0]
