...
> 노션용 버전 — 수식을 모두 일반 텍스트로 표기했습니다. 논문용 LaTeX는 부록 C 참조.
...
```
max  f(x) = ŷ·x − (δ/2)·xᵀΣx        s.t.  1ᵀx = 1,  x ≥ 0
 x
```
...
코너해 **x = e_i** (자산 i에 100%)에서, 예산제약을 지키며 자산 j로 비중 ε을 옮깁니다.

```
x(ε) = (1−ε)·e_i + ε·e_j        ε ∈ [0, 1]
```

**수익항**

```
ŷ·x(ε) = (1−ε)·ŷ_i + ε·ŷ_j = ŷ_i + ε·(ŷ_j − ŷ_i)
```

**위험항**

```
x(ε)ᵀ Σ x(ε) = (1−ε)²·Σ_ii + 2ε(1−ε)·Σ_ij + ε²·Σ_jj
```

ε으로 미분해 ε = 0에서 평가하면

```
d/dε [xᵀΣx] |_(ε=0) = −2·Σ_ii + 2·Σ_ij
```

### 2.3 유지 조건과 이탈 조건

```
df/dε |_(ε=0) = (ŷ_j − ŷ_i) − (δ/2)·(−2·Σ_ii + 2·Σ_ij)
              = (ŷ_j − ŷ_i) + δ·(Σ_ii − Σ_ij)
```

이 값의 부호가 모든 것을 결정합니다.

| df/dε \|₀ | 의미 | 결과 |
| --- | --- | --- |
| ≤ 0 | 움직이면 목적함수가 나빠짐 | **코너 유지** (몰빵) |
| > 0 | 움직이면 목적함수가 좋아짐 | **코너 이탈** (분산 시작) |

**코너 유지 조건** — e_i가 최적이려면 모든 j에 대해 `df/dε ≤ 0` 이어야 하므로

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [유지]   ŷ_i − ŷ_j  ≥  δ·(Σ_ii − Σ_ij)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

좌변은 "i를 고집해서 얻는 수익 우위", 우변은 "j로 분산해서 줄어드는 위험". **수익 우위가 더 크면 몰빵을 유지**합니다.

**코너 이탈 조건** — 위 부등호가 깨지는 경우입니다.

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [이탈]   ŷ_i − ŷ_j  <  δ·(Σ_ii − Σ_ij)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

δ > 0이고 Σ가 양반정치이므로 목적함수가 오목(concave)합니다. 따라서 이 1차 조건은 **전역 최적의 필요충분조건**입니다.

**이탈에 필요한 δ** — 이탈 조건을 δ에 대해 풀면

```
δ  >  δ* = Δŷ / (Σ_ii − Σ_ij)         where  Δŷ = ŷ_i − ŷ_j
```

즉 **δ가 δ*를 넘어서야 분산이 시작**됩니다.

핵심은 분모가 **Σ_ii가 아니라 Σ_ii − Σ_ij** 라는 점입니다. 두 자산이 같이 움직이는 부분(Σ_ij)만큼은 갈아타도 위험이 줄지 않기 때문입니다.
...
lookback 252일의 **일별** 수익률 표본공분산에 ridge 1e−4·I를 더합니다.
테스트 구간 32개 리밸런싱 시점에서 각각 계산해 평균냈습니다.

### 3.2 분모 (Σ_ii − Σ_ij)

| | Σ_ii | Σ_ij | Σ_ii − Σ_ij | 비율 | 평균 상관 ρ |
| --- | --- | --- | --- | --- | --- |
| ridge 포함 (최적화가 쓰는 값) | 3.821e−4 | 1.496e−4 | **2.325e−4** | **60.8%** | 0.354 |
| ridge 제외 (원시 상관) | 2.821e−4 | 1.496e−4 | 1.325e−4 | 47.0% | **0.543** |

- 평균 상관 ρ는 `C_ij = Σ_ij / √(Σ_ii · Σ_jj)` 의 비대각 원소(30×29 = 870개) 평균
- **산업 간 실제 평균 상관은 0.543**이며, 0.354는 ridge가 대각을 26% 부풀린 결과
- 비율 검산: `1 − Σ_ij/Σ_ii = 1 − 0.392 = 0.608` ✓
- 모든 분산이 같다면 이 비율은 정확히 `1 − ρ = 0.646`. 자산별 분산 차이로 0.608
...
교과서 MVO는 두 항을 **같은 기간 단위**로 맞춥니다. `Σ_N ≈ N · Σ_daily` 이므로

```
(δ/2)·Σ_daily = (δ_std/2)·Σ_N = (δ_std/2)·N·Σ_daily

  ⟹   δ = δ_std × N
```

통상 δ_std ≈ 5이므로 **δ ≈ 5 × 126 = 630**이 여기서의 "정상값"입니다.
...
①과 ②만으로는 δ ≈ 630~860이 설명됩니다. 현실적인 격차 Δŷ = 0.10~0.20을 가정하면 필요 δ는 430~860입니다. **관측된 5,000과는 6~12배 차이**가 남습니다.

### ③의 정체 — task loss에 예측 정확도 항이 없음

```
L = λ·(−Sharpe) + (1−λ)·MDD_real
```

이 loss는 ŷ에 **x\*를 통해서만** 영향을 받습니다. x\*는 ŷ의 **순서(ranking)** 에만 의존하고 크기에는 무관합니다 — 코너해에서는 argmax만 중요하기 때문입니다.

따라서 **ŷ가 실제 수익률과 같은 스케일일 이유가 없고**, 학습이 진행되며 자유롭게 팽창합니다. 억제 요인은 δ가 붙은 위험항뿐인데, δ=20에서 그것은 수익항의 2.5%에 불과해 제동이 걸리지 않습니다.

대조군이 이를 뒷받침합니다. **PTO-MDD는 MSE로 학습**해 ŷ가 실제 수익률에 고정(anchor)되며, HHI가 0.82~0.90으로 DFL-MVO(1.0)보다 덜 극단적입니다.
...
첫째, 산업들이 서로 비슷하게 움직여서(상관 0.54) **A에서 B로 갈아타도 위험이 기대만큼 줄지 않습니다.** 그만큼 저울추가 1.6배 더 무거워야 합니다.

둘째, 더 결정적으로 — **모델이 "이 자산이 저 자산보다 116%p 더 오른다"는 식의 과장된 예측**을 내놓습니다. 수익 쪽이 이렇게 무거우면 어지간한 저울추로는 움직이지 않습니다.
...
위험항이 사실상 꺼져 있으면 문제가 선형계획이 되고, **선형계획의 답은 항상 꼭짓점 — 즉 1등 자산 몰빵**입니다. δ를 20에서 2,000까지 100배 올려도 "어느 종목에 몰빵할지"만 바뀔 뿐 분산은 일어나지 않습니다.

실제로 이 구간에서 Calmar가 0.154 → 0.401 → 0.077로 요동치는데, 이는 분산 효과가 아니라 **종목이 바뀐 데서 오는 노이즈**입니다.
...
이 제약들이 단체(simplex)의 꼭짓점을 잘라내서, 몰빵이 제약을 위반하면 내부해로 밀려납니다. DFL-MDD의 HHI가 0.16~0.69로 나오는 이유이며, **n₁이 작을수록 HHI가 낮아지는** 관측과 일치합니다.

### 최종 결론

본 프레임워크에서 **δ는 실질적 위험 통제 수단이 아닙니다.** 그 역할은 drawdown 제약 n₁이 담당하며, 이것이 DFL-MDD가 DFL-MVO 대비 낙폭에서 우위를 보이는 메커니즘입니다. δ sweep 결과는 이 구조를 뒷받침하는 ablation 근거로 해석해야 합니다.
...
| 항목 | 현재 상태 | 확인 방법 |
| --- | --- | --- |
| Δŷ ≈ 1.16 | **추정값** (임계 δ에서 역산) | config 1개 재학습 후 ŷ 분포 직접 측정 (~30분) |
...
**(a) 스케일 정합** — `Σ_N = N · Σ_daily` 로 연율화하면 δ가 교과서 범위(2~10)로 복귀합니다. 코드 한 줄입니다.

```python
S = N * (np.cov(z_b.T) + 1e-4 * np.eye(m_dim))
```

**(b) 예측 스케일 고정** — loss에 MSE 항을 소량 섞어 ŷ를 실제 수익률에 anchor합니다.

```
L = λ·(−Sharpe) + (1−λ)·MDD + η·MSE(r̂, r)
```
...
노션에서는 텍스트로 두고, 논문 작성 시 아래를 사용하세요.
...
```latex
x(\varepsilon)^\top \Sigma x(\varepsilon)
= (1-\varepsilon)^2\Sigma_{ii} + 2\varepsilon(1-\varepsilon)\Sigma_{ij} + \varepsilon^2\Sigma_{jj}
```
...
```latex
\hat{y}_i - \hat{y}_j \;\ge\; \delta\left(\Sigma_{ii} - \Sigma_{ij}\right)
```
...
```latex
\delta^{*} = \frac{\Delta\hat{y}}{\Sigma_{ii} - \Sigma_{ij}},
\qquad \Delta\hat{y} \equiv \hat{y}_i - \hat{y}_j
```
