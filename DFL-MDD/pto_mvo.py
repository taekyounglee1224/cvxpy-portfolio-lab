...
def backtest_pto_mvo(pred_model, rebal_samples, N, d, C,
    ...
    for i, (z_np, r_np) in enumerate(rebal_samples):
        ...
        with torch.no_grad():
            r_hat = pred_model(z)[0].numpy()           # (N, m)

        mu    = r_hat.mean(axis=0)                     # (m,)
        Sigma = np.cov(r_hat.T) + 1e-6 * np.eye(m)    # (m, m)
        w     = _solve_mvo(mu, Sigma, lam_mvo, x_min, x_max)

        y_real = np.cumsum(r_np, axis=0)               # (N, m)
        w_real = y_real @ w                             # (N,)
...
def plot_pnl(bt_results: list, horizon: int, figsize=(12, 6)):
    ...
    # -- PnL --
    ax1.plot(x, pv, color="steelblue", linewidth=1.8, label="PTO-MVO Portfolio")
